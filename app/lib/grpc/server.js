 ...
 * @Last Modified time: 2017-12-19 10:36:18
 */
  ...
  invoke (call, callback) {
    ...
    return authToken.decodeInsideToken(access_token)
      .then(tokenDate => {
        ...
        let args = JSON.parse(call.request.jsonArgs);
      ...
      .catch(err => {
        util.log.info(err);
        if (err instanceof Array) return callback({code: err[0], message: err[1], status: grpc.status.INTERNAL});
        return callback(Object.assign(err, {code: 500, status: grpc.status.INTERNAL}));
      });
  }
  start() {
