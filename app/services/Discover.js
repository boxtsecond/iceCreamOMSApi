...
class Discover {
  ...
  /**
   * grpc服务 修改发现模块的排序分值
   * @param {Array} [{discoverid, score}, {}]
   */
  updateDiscoverDisplayScore (sid, idAndScoreList) {
    if (!(idAndScoreList instanceof Array)) idAndScoreList = [idAndScoreList];
    return this.mysqlDiscoverModel.updateDiscoverDisplayScore(sid, idAndScoreList)
      .then(res => this.redisDiscoverModel.updateDiscoverDisplayScore( sid, idAndScoreList));
