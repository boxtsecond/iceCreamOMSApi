...
class discoverModel {
  ...
  /**
   * 修改发现模块的排序分值
   * @param {Array} [{discoversid, score}, {}]
   */
  updateDiscoverDisplayScore(sid, idAndScoreList) {
    return this.wclient.transaction(trx => {
      return Promise.map(idAndScoreList, idAndScore => trx(this.displayTable)
        .where({ sid, id: idAndScore.id })
        .update({ score: idAndScore.score })
      );
    });
  }
