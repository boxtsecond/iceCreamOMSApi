...
class goodsModel {
  ...
  updateGoodsByGoodsSn (goods_sn, goods_num, sell_num) {
    return this.wclient(this.goodsTable).update({goods_num, sell_num}).where({goods_sn: goods_sn});
  }
  
  updateGoodsLimit (sid, uid, goods_sn, updateCount, trx) {
    let mtime = eutil.getTimeSeconds();
    if(trx) return trx.raw('insert into ?? set sid = ?, uid = ? ,goods_sn = ?, goods_count = ? on duplicate key update goods_count = goods_count + ?, mtime = ?', [this.goodsLimitTable, sid, uid, goods_sn, Math.abs(updateCount), updateCount, mtime]);
    else return this.wclient.raw('insert into ?? set sid = ?, uid = ? ,goods_sn = ?, goods_count = ? on duplicate key update goods_count = goods_count + ?, mtime = ?', [this.goodsLimitTable, sid, uid, goods_sn, Math.abs(updateCount), updateCount, mtime]);
  }
  ...
  upsertGoodsBill (goodsbill) {
    return this.wclient.raw('insert into ?? set sid = ?, report_type = ? ,goods_sn = ?, is_digital = ?, goods_id = ?, goods_name = ?, goods_price = ?, total_sell_price = ?, sell_num = ?, y_month = ?, ctime = ?, mtime = ? on duplicate key update total_sell_price = ?, sell_num = ?, mtime = ?', [`${this.goodsBillTable}_${goodsbill.y_month}`, goodsbill.sid, goodsbill.report_type, goodsbill.goods_sn, goodsbill.is_digital, goodsbill.goods_id, goodsbill.goods_name, goodsbill.goods_price, goodsbill.total_sell_price, goodsbill.sell_num, goodsbill.y_month, Number(goodsbill.ctime), goodsbill.mtime, goodsbill.total_sell_price, goodsbill.sell_num, goodsbill.mtime]);
  }
  ...
  //ctime means the user last view time
  getUpdateTipsByDiscoverIdAndCtime(sid, discover_displayid, lastctime) {
    let select = {
      raw: `select d.id, d.sid, d.discover_displayid, d.goodsid, g.onsale_time from (select id,onsale_time from ${this.goodsTable} where sid = ? and onsale_time between ? and ?) g join (select id, sid, discover_displayid, goodsid from ${this.discoverGoodsTable} where sid = ? and discover_displayid = ?) d on d.goodsid=g.id`,
      params: [sid, lastctime, eutil.getTimeSeconds(), sid, discover_displayid]
    };
    return this.rclient.raw(select.raw, select.params).then(arr => {return arr[0];});
  }
  
  getGoodsLastctime(sid, discover_displayid) {
    let findCondition = {sid};
    if(discover_displayid && discover_displayid != 0)  findCondition.discover_displayid = discover_displayid;
    return this.rclient(this.discoverGoodsTable).select('discover_displayid', 'ctime').where(findCondition).orderBy('ctime', 'desc').limit(1).then(ctimeArr => {
      if(ctimeArr && ctimeArr.length) return ctimeArr[0];
      else return false;
    });
  }
