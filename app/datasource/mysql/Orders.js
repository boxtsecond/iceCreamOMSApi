...
class masterModel {
  ...
  deleteOrder (order_no, sid, uid, creater, isRefund, trx) {
    let updateObj = {order_status: 7, note: `${creater} 删除订单信息`, mtime: eutil.getTimeSeconds()};
    if(isRefund) updateObj.refund_status = 2, updateObj.note += ', 并且将星星退回';
    if(!trx) return this.wclient(this.orderTable).where({order_no, sid, uid}).update(updateObj).limit(1);
    else return this.wclient(this.orderTable).where({order_no, sid, uid}).update(updateObj).limit(1).transacting(trx);
  }
  
  deleteOrderDetail (order_no, sid, uid, trx) {
    if(!trx) return this.wclient(this.orderDetailTable).where({order_no, sid, uid}).del().limit(1);
    else return this.wclient(this.orderDetailTable).where({order_no, sid, uid}).del().limit(1).transacting(trx);
  }
}
